...
function ComparisonReportDocument({ comparison }: { comparison: ComparisonDoc }) {
  ...
  const showFinancials = procurement || hasFinancialData(comparison.financials);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subtitle}>
          {getModeLabel(comparison.mode)} · Generated {createdAt}
        </Text>
        ...
        {(procurement || comparison.lineItemDiffs.length > 0) && (
          <>
            <Text style={styles.sectionTitle}>
              {procurement ? "Line Item Comparison" : "Field Comparison"}
            </Text>
            <View style={styles.table}>
              <View style={styles.tableHeaderRow}>
                <Text style={[styles.cellDesc, styles.headerCell]}>Description</Text>
                <Text style={[styles.cellSm, styles.headerCell]}>{procurement ? "PO Qty" : "Doc A Qty"}</Text>
                <Text style={[styles.cellSm, styles.headerCell]}>{procurement ? "Inv Qty" : "Doc B Qty"}</Text>
                <Text style={[styles.cellSm, styles.headerCell]}>{procurement ? "PO Price" : "Doc A Value"}</Text>
                <Text style={[styles.cellSm, styles.headerCell]}>{procurement ? "Inv Price" : "Doc B Value"}</Text>
              ...
              {comparison.lineItemDiffs.map((item) => (
                <View style={styles.tableRow} key={item.id}>
                  ...
                  <Text style={[styles.cellSm, styles.cellText]}>{item.poPrice != null ? `Rs. ${item.poPrice.toFixed(2)}` : "—"}</Text>
                  <Text style={[styles.cellSm, styles.cellText]}>{item.invPrice != null ? `Rs. ${item.invPrice.toFixed(2)}` : "—"}</Text>
